...
@interface LXRouteInfo : NSObject
{
...
+ (nullable LXRouteInfo*) getRoute:(struct rt_msghdr2 *)rtm;
...
+ (nullable LXRouteInfo*) getRoute:(struct rt_msghdr2 *)rtm
{
    ...
    if(rtm->rtm_addrs & RTA_DST)
    {
        ...
        {
            route = [[LXRouteInfo alloc] initWithRtm:rtm];
        }
    }
...
+ (nullable NSString *)wiFiRouterAddress {
    ...
    @try {
        ...
        NSMutableArray *routerArray = [LXRouteInfo getRoutes];
        // Run through the array
        for(int i = 0; i < (int)[routerArray count]; i++)
        {
            // Set the router info
            LXRouteInfo* router = (LXRouteInfo*)[routerArray objectAtIndex:i];
