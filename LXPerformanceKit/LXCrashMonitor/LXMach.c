 ...
 */
bool lxmach_i_VMStats(vm_statistics_data_t* const vmStats,
...
uint64_t lxmach_freeMemory(void)
{
    ...
    vm_size_t pageSize;
    if(lxmach_i_VMStats(&vmStats, &pageSize))
    {
...
const char* lxmach_exceptionName(const exception_type_t exceptionType)
{
...
const char* lxmach_kernelReturnCodeName(const kern_return_t returnCode)
{
    switch (returnCode)
    {
...
bool lxmach_fillState(const thread_t thread,
                      ...
                      const mach_msg_type_number_t stateCount)
{
    ...
    if(kr != KERN_SUCCESS)
    {
        printf("thread_get_state: %s", mach_error_string(kr));
        return false;
    }
    return true;
}
...
thread_t lxmach_machThreadFromPThread(const pthread_t pthread)
{
    ...
    if(lxmach_copyMem(&threadStruct->kernel_thread, &machThread, sizeof(machThread)) != KERN_SUCCESS)
    {
        printf("Could not copy mach thread from %p", threadStruct->kernel_thread);
        return 0;
    }
    return machThread;
}
...
pthread_t lxmach_pthreadFromMachThread(const thread_t thread)
{
    ...
    for(int i = 0; i < 50; i++)
    {
        ...
        if(lxmach_copyMem(&threadStruct->plist.tqe_next, &threadStruct, sizeof(threadStruct)) != KERN_SUCCESS)
        {
            break;
        }
    }
...
bool lxmach_getThreadName(const thread_t thread,
                          char* const buffer,
                          size_t bufLength)
{
...
bool lxmach_getThreadQueueName(const thread_t thread,
                               char* const buffer,
                               size_t bufLength)
{
    // WARNING: This implementation is no longer async-safe!
...
{
    return lxmach_suspendAllThreadsExcept(NULL, 0);
}
...
bool lxmach_suspendAllThreadsExcept(thread_t* exceptThreads, int exceptThreadsCount)
{
    kern_return_t kr;
    const task_t thisTask = mach_task_self();
    const thread_t thisThread = lxmach_thread_self();
    thread_act_array_t threads;
    mach_msg_type_number_t numThreads;
...
{
    return lxmach_resumeAllThreadsExcept(NULL, 0);
}
...
kern_return_t lxmach_copyMem(const void* const src,
                             void* const dst,
...
size_t lxmach_copyMaxPossibleMem(const void* const src,
                                 void* const dst,
                                 const size_t numBytes)
{
    ...
    if(lxmach_copyMem(src, dst, 1) != KERN_SUCCESS)
    {
        return 0;
    }
    else if(numBytes <= 1)
    ...
    for(;;)
    {
        ...
        if(lxmach_copyMem(pSrc, pDst, (size_t)copyLength) == KERN_SUCCESS)
        {
...
double lxmach_timeDifferenceInSeconds(const uint64_t endTime,
                                      const uint64_t startTime)
{
    ...
    if(conversion == 0)
    {
        ...
        if(kr != KERN_SUCCESS)
        {
            printf("mach_timebase_info: %s", mach_error_string(kr));
            return 0;
        }
                      ...
                      vm_size_t* const pageSize)
{
    ...
    {
        printf("host_page_size: %s", mach_error_string(kr));
        return false;
    }
    ...
    if(kr != KERN_SUCCESS)
    {
        printf("host_statistics: %s", mach_error_string(kr));
        return false;
    }
